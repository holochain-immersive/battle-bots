...
/**
 * Magenta is running a very simple decision making algorithm:
 *
 ...
 */
pub fn magenta(game_state: &GameState, bot_position: Position) -> Actuators {
    ...
    if let Some(bot) = maybe_bot {
        if let Some(adjacent_bot_direction) = adjacent_bot(game_state, bot_position) {
            ...
            let result = match possible_adj_bot {
                Some(adj_bot) => {
                    match is_bot_color(&adj_bot, Color::Magenta) {
                        false => Some(rotation),
                        true => None,
                    }
                },
...
fn is_bot_color(bot: &Bot, color: Color) -> bool {
    if let Color::Magenta = bot.color {
        return true;
    }
    else {
        return false;
...
fn get_closest_enemy(
    ...
    for (position, bot) in game_state.bots.iter() {
        if are_positions_equal(bot_position.x, bot_position.y, position.x, position.y) && !is_bot_color(bot, Color::Magenta) {
            match closest_enemy {
                Some(Position {
                    x: closest_x,
                    y: closest_y,
